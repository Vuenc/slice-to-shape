...
from ssm_thyroid_dataset_originalmethod import SSMvoxelDataset
...
def train(
    ...
    save_dir="saves/",
    device: str="cuda:0",
    ultrasound_patches_shape=(32,32,32),
    sdf_patches_shape=(32,32,32),
    ...
    negative_sample_from_p_furthest = 0.2,
    ...
    if model is None:
        ...
        # model = SdfVoxelMatchNet(embedding_size=embedding_size, normalize_embeddings=normalize_embeddings,
        #     ultrasound_patches_shape=ultrasound_patches_shape, sdf_patches_shape=sdf_patches_shape, encoder_type = "3dunet-encoder-decoder").to(device)        
    ...
    train_dataloader = DataLoader(train_dataset, 
        batch_sampler=BlockRandomBatchSampler(train_dataset.get_block_sizes(), batch_size, allow_block_overlap=False))
    ...
    log_comment = "_" + "_".join((key + "=" if key is not None else "") + str(value) for key, value in hyperparams if value is not None)
    writer = SummaryWriter(None, log_comment) if tensorboard else None
    ...
    for epoch in tqdm(range(epoch_offset, epoch_offset + epochs)):
        ...
        if save_every_n_epochs is not None and epoch % save_every_n_epochs == save_every_n_epochs - 1:
            torch.save(model, Path(save_dir) / ("model_%s_%s.obj" % (time_string, log_comment)))
        val_loss=np.mean(statistics["loss/validation"])
        if val_loss<best_loss:
            torch.save(model, Path(save_dir) / ("best_model_%s_%s.obj" % (time_string, log_comment)))
            best_loss=val_loss
            stopping_counter=0
        else:
            stopping_counter+=1
